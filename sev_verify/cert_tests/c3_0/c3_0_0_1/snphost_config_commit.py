...
def steps() -> list[Step]:
    ...
    return [
        Step(name="show-tcb", type="setup", runs_on="host",
             command="snphost show tcb",
             expected_result="exit_code:0", timeout=10),
        Step(name="config-set-lower", type="required", runs_on="host",
             command=f"{_config_set(lo_bl, lo_tee, lo_snp, lo_ucode, fmc, 0)} && {_verify_cmd('verify-differ')}",
             expected_result="exit_code:0", timeout=10),
        Step(name="config-reset", type="required", runs_on="host",
             command=f"snphost config reset && {_verify_cmd('verify-match')}",
             expected_result="exit_code:0", timeout=10),
        Step(name="config-set-mask-chip-id", type="required", runs_on="host",
             command=_config_set(bl, tee, snp, ucode, fmc, 1),
             expected_result="exit_code:0", timeout=10),
        Step(name="config-set-mask-chip-key", type="required", runs_on="host",
             command=_config_set(bl, tee, snp, ucode, fmc, 2),
             expected_result="exit_code:0", timeout=10),
        Step(name="config-set-mask-both", type="required", runs_on="host",
             command=_config_set(bl, tee, snp, ucode, fmc, 3),
             expected_result="exit_code:0", timeout=10),
        Step(name="config-reset-masks", type="required", runs_on="host",
             command=f"snphost config reset && {_verify_cmd('verify-match')}",
             expected_result="exit_code:0", timeout=10),
        Step(name="commit", type="required", runs_on="host",
